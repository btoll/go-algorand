...
ssh -i ReleaseBuildInstanceKey.pem -A ubuntu@"$INSTANCE" bash ./scripts/release/build/stage/upload/task.sh
...
aws s3 sync --exclude dev* --exclude master* --exclude nightly* --exclude stable* --acl public-read pkg/"$FULLVERSION" s3://algorand-builds/channel/"$CHANNEL"/foo/
