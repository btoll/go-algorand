...
ssh -i ReleaseBuildInstanceKey.pem ubuntu@"$INSTANCE" mkdir node_pkg
...
ssh -i ReleaseBuildInstanceKey.pem ubuntu@"$INSTANCE" bash setup.sh "$BRANCH" "$CHANNEL" "$FULLVERSION"
